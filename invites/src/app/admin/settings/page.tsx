...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<FlashParams> }) {
  ...
  return (
    <>
      <PageHeader title="Settings" subtitle="Business details, payment accounts, service levels, policies and message templates." actions={<><Link href="/admin/settings/pricing" className="btn btn-secondary btn-sm">Packages & add-ons</Link>{can(user.role, 'templates.edit') && <Link href="/admin/settings/canva" className="btn btn-secondary btn-sm">Canva</Link>}{can(user.role, 'users.manage') && <Link href="/admin/settings/users" className="btn btn-secondary btn-sm">Staff accounts</Link>}{can(user.role, 'audit.view') && <Link href="/admin/settings/audit" className="btn btn-secondary btn-sm">Audit trail</Link>}{can(user.role, 'audit.view') && <Link href="/admin/settings/signins" className="btn btn-secondary btn-sm">Sign-ins</Link>}</>} />
      <Flash {...sp} />
