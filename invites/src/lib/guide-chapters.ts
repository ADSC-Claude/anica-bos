...
export const CHAPTERS: Chapter[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'Everything your package asks for is also on one document — the details sheet, linked under the Get started list — as a Word file to fill in offline, send back over Messenger, or read as a list of what to gather before you sit down. It has the same parts as this tab, for your package and no more.',
      'A list of names — the ninongs and ninangs, the entourage, the programme — takes a pasted list: press Paste a list, paste one name per line (a pair per line for the sponsors, as they come out of Excel), and the rows fill in below. Arrange shows the list as one line per name, to drag into order — on a phone too. The Sheet buttons take the same list as an Excel file, out and back.',
