...
export function pageNeeds({ doc, occasion, content, weights, lengths, shop }: Look): Need[] {
  ...
  if (weights) {
     ...
     * everything else.
     */
    const heavy = reachablePages(doc).flatMap((pg) => (pg.elements ?? []).flatMap((e) => {
    ...
    const player = reachablePages(doc).some((pg) => (pg.elements ?? []).some((e) => e.kind === 'anim')) ? LOTTIE_PLAYER_BYTES : 0;
