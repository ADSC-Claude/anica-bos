 ...
 * Resolve one to the other here rather than making every caller care.
 *
 * Returns null when the payment cannot be found, which the caller must treat as
 * "settle this by hand" rather than as a failure to record.
 */
...
/**
 * A refund on an e-wallet can sit `pending` for days before it settles, so the
 * status is reported rather than assumed; the `payment.refunded` webhook is
...
export async function createRefund(opts: {
  ...
  if (isSimulated()) {
    return {
      id: `sim_refund_${opts.reference || opts.paymentId}`,
      status: 'succeeded',
      simulated: true,
    };
  }
    ...
    body: JSON.stringify({
      data: {
        attributes: {
          ...
          // Capped at 255 characters and rejected outright beyond it, so trim
          // rather than risk losing the whole refund.
