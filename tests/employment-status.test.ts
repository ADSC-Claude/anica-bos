...
test('separating sets a date, and keeps the record', async () => {
  ...
  assert.equal(e.separatedAt?.toISOString(), lastDay.toISOString());
  assert.equal(e.separationReason, 'Resigned');
...
test('the delete fallback sets status and active together', async () => {
  await prisma.employee.update({
    where: { id: employeeId },
    data: { status: 'ACTIVE', active: true, separatedAt: null },
  });
  ...
  assert.ok(e.separatedAt);
});

test('taking someone back on clears the separation date', async () => {
  await prisma.employee.update({
    where: { id: employeeId },
    data: { status: 'ACTIVE', active: true, separatedAt: null, separationReason: '' },
  });
  const e = await prisma.employee.findUniqueOrThrow({ where: { id: employeeId } });
  assert.equal(e.separatedAt, null);
  assert.equal(e.separationReason, '');
});
